...
class PlayoffCreation(object):
    ...
    def get_players_by_id(self):
        ...
        players_id = {}
        count_key = 0
        number_players = self.get_number_players(target)
    ...
    def delete_teams_instances(self):
        ...
        teams_instance = self.get_teams_by_id(target)
    ...
    def delete_player_instances(self):
        ...
        players_instance = self.get_players_by_id(target)
    ...
    def import_teams_instance(self):
        with open(self.file_path + "teamsInstances.json", "r") as file:
            ...
            for key, value in teams_instance.items() :
                try:
                    control = self.target.get('/admin/teams/' + key, {})
                    if control is not None:
                        self.target.delete('/admin/teams/' + key, {})
                except PlayoffException as err:
                    if not err.message == "Team " + key + " not found":
                        self.target.delete('/admin/teams/' + key, {})
                    else:
                        pprint(err.message)
